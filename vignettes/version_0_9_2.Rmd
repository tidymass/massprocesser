...
First, you need to set the work directory to the folder which you used to run the `process_data()` function. For example, in our [example for `process_Data`](https://tidymass.github.io/massprocesser/articles/raw_data_processing.html), we set the work directory in `example/POS`, so here, we also set work directory in this folder.
