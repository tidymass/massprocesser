#' Conflicts between the massprocesser and other packages
#'
#' This function lists all the conflicts between packages in the massprocesser
...
massprocesser_conflicts <- function() {
  ...
  tidy_names <- paste0("package:", massprocesser_packages())
  conflicts <- purrr::keep(conflicts, ~ any(.x %in% tidy_names))
  
  conflict_funs <- purrr::imap(conflicts, confirm_conflict)
  conflict_funs <- purrr::compact(conflict_funs)
  
  structure(conflict_funs, class = "massprocesser_conflicts")
}

massprocesser_conflict_message <- function(x) {
  if (length(x) == 0) return("")
  
  header <- cli::rule(
    left = crayon::bold("Conflicts"),
    right = "massprocesser_conflicts()"
  )
...
#' @export
print.massprocesser_conflicts <- function(x, ..., startup = FALSE) {
  cli::cat_line(massprocesser_conflict_message(x))
}
